...
app.delete("/deleteuser/:id", (req, res) => {
  ...
  const index=users.findIndex(ind=>ind.id==uid)
  if(index==-1){
    res.status(400).json({status:"fail",message:"user not found",data:users[index]})
  }
  else{
    const deldata = users.splice(index,1)
    res.status(200).json({status:"success",message:"user data deletd successfully",data:deldata})
  }
})
